...
function wpsp_pro_process_simple_like() {
	...
	if ( $post_id != '' ) {
		...
		if ( !wpsp_pro_already_liked( $post_id ) ) { // Like the post
...
function wpsp_get_love_button( $post_id ) {
	$output = '';
	$nonce = wp_create_nonce( 'wpsp-li-nonce' ); // Security
